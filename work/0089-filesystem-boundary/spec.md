...
4. A second or replacement claim receives fresh data even if the backend reuses physical infrastructure. Parent/child lineage does not expose the parent's directory. An allocation may not silently reuse another claim's files.
...
- The MVP output path is explicit export **before termination**. The worker prepares a diff (including an explicit choice about untracked files), commit bundle or regular artifact under W, then submits its bytes through the existing authorized output/tool boundary to a caller-owned destination. A commit hash alone is not exported content unless the commit has already reached that destination. The directory itself is never the retained output.
