...
- Known adapter gap (verified at `internal/runtime/agentsandbox/adapter.go:242-272`): the spike's `StartClaim` polls until the upstream SandboxClaim reports the `Ready=True` condition, then marks the claim Running, so it treats readiness as work start. Under this Ticket that observation maps to Bound only; the explicit start boundary is reported as unsupported by the adapter until a real worker start mechanism exists.
- Real-cluster evidence: no verified test cluster is available in the Owner's current local environment. Provider-behavior changes will be recorded as an environment blocker unless the PR #99 / Ticket #66 owners can run the integration gate on their kind substrate; they are also natural adapter-side Reviewer candidates, subject to team confirmation.
